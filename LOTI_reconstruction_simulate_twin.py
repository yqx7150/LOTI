...
def get_pc_sampler(sde, sde_T, shape, predictor, corrector, inverse_scaler, snr,
                   n_steps=1, probability_flow=False, continuous=False, continuous_T=False,
  ...
  """
    ...
    corrector_update_fn = functools.partial(shared_corrector_update_fn,
                                            ...
                                            snr=snr,
                                            n_steps=n_steps)
    predictor_update_fn_T = functools.partial(shared_predictor_update_fn,
                                              sde=sde_T,
                                              predictor=predictor,
                                              probability_flow=probability_flow,
                                              continuous=continuous_T)
    corrector_update_fn_T = functools.partial(shared_corrector_update_fn,
                                              sde=sde_T,
                                              corrector=corrector,
                                              continuous=continuous_T,
                                              snr=snr,
    ...
    def pc_sampler(model, model_T, data, mask1, mask2, data_ob1, data_ob2,k):
    ...
    """
        with torch.no_grad():
            ...
            data_ob1 = data_ob1.permute(2, 0, 1)
            data_ob2 = data_ob2.permute(2, 0, 1)
            data_ob1 = (data_ob1 - data_ob1.min()) / (data_ob1.max() - data_ob1.min())
            data_ob2 = (data_ob2 - data_ob2.min()) / (data_ob2.max() - data_ob2.min())
            data_ob_dc1 = backward(data_ob1[0, :, :], mask1) + backward(data_ob2[0, :, :], mask2)
            data_ob_dc2 = backward(data_ob1[1, :, :], mask1) + backward(data_ob2[1, :, :], mask2)
            data_ob_dc3 = backward(data_ob1[2, :, :], mask1) + backward(data_ob2[2, :, :], mask2)
            data_ob_dc = torch.stack((data_ob_dc1, data_ob_dc2, data_ob_dc3), dim=0).unsqueeze(0)
            data_ob_dc = (data_ob_dc - data_ob_dc.min()) / (data_ob_dc.max() - data_ob_dc.min())
            ...
            for i in range(800):
                ...
                x_r_red = data_ob_dc[0, 0, :, :] - x_mean[0, 0, :, :]
                x_r_green = data_ob_dc[0, 1, :, :] - x_mean[0, 1, :, :]
                x_r_blue = data_ob_dc[0, 2, :, :] - x_mean[0, 2, :, :]
                x_r = torch.stack((x_r_red, x_r_green, x_r_blue), dim=0).unsqueeze(0)


                x_mean = torch.stack((x_r[0, 0, :, :], x_r[0, 1, :, :], x_r[0, 2, :, :]), dim=0).unsqueeze(0)
                x, x_mean = predictor_update_fn_T(x_mean, vec_t, model=model_T)
                x1 = x
                x2 = x
                x3 = x
                x1, x2, x3, x_mean = corrector_update_fn_T(x1, x2, x3, x_mean, vec_t, model=model_T)

                x_r_red = data_ob_dc[0, 0, :, :] - x_mean[0, 0, :, :]
                x_r_green = data_ob_dc[0, 1, :, :] - x_mean[0, 1, :, :]
                x_r_blue = data_ob_dc[0, 2, :, :] - x_mean[0, 2, :, :]
                x_mean = torch.stack((x_r_red, x_r_green, x_r_blue), dim=0).unsqueeze(0)
                ...
                psnr = compare_psnr(np.abs(x_mean_save_cv[81: 176, 81: 176,:]) * 255,np.abs(data_ori[81: 176, 81: 176,:]) * 255, data_range=255)
                ssim = compare_ssim(np.abs(x_mean_save_cv[81: 176, 81: 176,:]), np.abs(data_ori[81: 176, 81: 176,:]),multichannel=True, data_range=1)
                psnr_1 = compare_psnr(np.abs(x_mean_save_cv) * 255, np.abs(data_ori) * 255, data_range=255)
                ...
                # 保存图像到目标文件夹
                output_path = os.path.join(output_folder, f'real-time_reconstruction_{i}.png')
                cv2.imwrite(output_path, x_mean_save_img * 255)

                print('iterations:', i, 'psnr:', psnr, 'ssim:', ssim)
                psnr_n.append(psnr)
                ssim_n.append(ssim)

            return x_mean_save_img, psnr, ssim, psnr_1, ssim_1
