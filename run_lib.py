...
def train(config, workdir):
  ...
  # Create directories for experimental logs 创建实验的目录
  sample_dir = os.path.join(workdir, "samples")
  tf.io.gfile.makedirs(sample_dir) #创建文件夹
  ...
  ema = ExponentialMovingAverage(score_model.parameters(), decay=config.model.ema_rate)#指数移动平均
  optimizer = losses.get_optimizer(config, score_model.parameters()) #adam优化
  state = dict(optimizer=optimizer, model=score_model, ema=ema, step=0) #设置保存模型的字典

  # Create checkpoints directory #创建模型的目录
  checkpoint_dir = os.path.join(workdir, "checkpoints")
  # Intermediate checkpoints to resume training after pre-emption in cloud environments #续训练
  ...
  dataset = LoadDataset('/home/qgl/桌面/twin_image/data/twin_train5',tran_transform)
  ...
  # Create data normalizer and its inverse 归一化与反归一化
  ...
  # Build one-step training and evaluation functions 构建一步训练和评估函数
  ...
  # Building sampling functions 构建采样函数
    ...
    for i, X in enumerate(dataloader):
      ...
      if step_end != 0 and step_end % config.training.snapshot_freq == 0 or step_end== num_train_steps:
        ...
        if config.training.snapshot_sampling:
          ...
          image_grid = make_grid(sample, nrow, padding=2)#将采样的样本拼成一幅图片
