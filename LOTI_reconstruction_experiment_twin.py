...
import numpy as np
import abc
from pixel import pixel, pixel3, pixel_3,pixel3_2,pixel3_3
...
def get_pc_sampler(sde, sde_T, shape, predictor, corrector, inverse_scaler, snr,
  ...
  """
    ...
    def pc_sampler(model, model_T, mask1, mask2, data_ob1, data_ob2):
    ...
    """
        with torch.no_grad():
            ...
            data_ob_dc = torch.stack((data_ob_dc1, data_ob_dc2, data_ob_dc3), dim=-1) #(1,3,1600,1600)
            data_ob_dc = (data_ob_dc - data_ob_dc.min()) / (data_ob_dc.max() - data_ob_dc.min())
            data_ob_dc=data_ob_dc.permute(2,0,1).unsqueeze(0)
            ...
            for i in range(1000):
                ...
                x_mean = torch.stack((x_r[0, 0, :, :], x_r[0, 1, :, :], x_r[0, 2, :, :]), dim=0).unsqueeze(0).float()
                ...
                x_r_green = data_ob_dc[0, 1, :, :] - x_mean[0, 1, :, :]
                x_r_blue = data_ob_dc[0, 2, :, :] - x_mean[0, 2, :, :]
                x_mean = torch.stack((x_r_red, x_r_green, x_r_blue), dim=0).unsqueeze(0).float()
